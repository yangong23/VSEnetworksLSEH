...
class EncoderImagePrecomp(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, use_abs=False, no_imgnorm=False):
        ...
        self.fc = nn.Linear(36 * 2048, embed_size)
...
class ContrastiveLossLSEH(nn.Module):
    ...
    def forward(self, im, s, ids, img_ids, svd):
        ...
        if torch.cuda.is_available():
            IL = IL.cuda()
        LsaMargin = SeMargin.masked_fill_(IL, 0)
...
class VSE(object):
    ...
    def __init__(self, opt):
        ...
        self.criterionLSEH = ContrastiveLossLSEH(margin=opt.margin,
                                         measure=opt.measure,
                                         max_violation=opt.max_violation)
    ...
    def forward_loss(self, img_emb, cap_emb, ids, img_ids, lsa, epoch, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """

        # loss, lossS, lossIm = self.criterion(img_emb, cap_emb)

        loss, lossS, lossIm = self.criterionLSEH(img_emb, cap_emb, ids, img_ids, lsa)

        self.logger.update('Le', loss.data.item(), img_emb.size(0))
        self.logger.update('LeS', lossS.data.item(), img_emb.size(0))
        self.logger.update('LeIm', lossIm.data.item(), img_emb.size(0))

        return loss

    def train_emb(self, images, captions, lengths, ids, img_ids, lsa, epoch, *args):
        ...
        loss = self.forward_loss(img_emb, cap_emb, ids, img_ids, lsa, epoch)

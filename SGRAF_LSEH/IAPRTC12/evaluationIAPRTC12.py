...
class LogCollector(object):
    ...
    def __str__(self):
        ...
        """
        s = ''
        for i, (k, v) in enumerate(self.meters.items()):#for k, v in self.meters.items():
            if i > 0:
                s += '  '
            s += k + ' ' + str(v)
        return s
...
def t2i(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    for index in range(npts):
        ...
        for i in range(1):
            inds = np.argsort(sims[index + i])[::-1]
            ranks[index + i] = np.where(inds == index)[0][0]
            top1[index + i] = inds[0]
...
if __name__ == '__main__':
    evalrank("./runs/iaprtc12_SGR/checkpoint/model_best.pth.tar",
             data_path="/home/lunet/coyg4/data/iaprtc12", split="test", fold5=False)

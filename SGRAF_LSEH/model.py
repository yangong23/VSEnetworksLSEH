...
class SGRAF(object):
    ...
    def forward_loss(self, sims, ids, img_ids, lsa, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        # loss, lossS, lossIm = self.criterion(sims)

        loss, lossS, lossIm = self.criterionLSEH(sims, ids, img_ids, lsa)
        self.logger.update('Loss', loss.item(), sims.size(0))
        self.logger.update('LeS', lossS.item(), sims.size(0))
        self.logger.update('LeIm', lossIm.item(), sims.size(0))

        return loss

    def train_emb(self, images, captions, lengths, ids, img_ids, lsa, *args):
        ...
        loss = self.forward_loss(sims, ids, img_ids, lsa)

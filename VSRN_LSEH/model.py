...
class ContrastiveLossLSEH(nn.Module):
    ...
    def __init__(self, margin=0, a=0.185, b=0.025, measure=False, max_violation=False):
    ...
    def forward(self, im, s, dv, ids, img_ids):
        # compute image-sentence score matrix
        scores = self.sim(im, s)
        diagonal = scores.diag().view(im.size(0), 1)
        d1 = diagonal.expand_as(scores)
        ...
        # LSEH is sensitive to the image's multiple corresponding description appearing in a same mini-batch,
        # decrease the similarities of unpaired corresponding image-description as 0 if the above happens
        ids = np.array(ids)
        ...
        SeScores = SeScores * self.b
        SeMargin = SeScores + self.a
...
class VSRN(object):
    ...
    def __init__(self, opt):
        ...
        self.criterion = ContrastiveLossLSEH(margin=opt.margin,
                                                a=opt.a,
                                                b=opt.b,
                                         measure=opt.measure,
                                         max_violation=opt.max_violation)
    ...
    def forward_loss(self, img_emb, cap_emb, dv, ids, img_ids, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        if torch.cuda.is_available():
            dv = dv.cuda()

        loss = self.criterion(img_emb, cap_emb, dv, ids, img_ids)
    ...
    def train_emb(self, images, captions, lengths, ids, img_ids, caption_labels, caption_masks, dv, *args):
        ...
        self.optimizer.zero_grad()
        retrieval_loss = self.forward_loss(img_emb, cap_emb, dv, ids, img_ids)

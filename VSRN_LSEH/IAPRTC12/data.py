...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt):
        ...
        with open(loc + '%s_caps.txt' % data_split, 'rb') as f:
            for line in f:
                ...
                tokens = nltk.tokenize.word_tokenize(str(line.strip(), 'utf8').lower().encode('utf-8').decode('utf-8'))
        ...
        if data_split == 'train':
            self.dv = np.loadtxt(loc + '%s_svd.txt' % data_split, dtype=float, delimiter=',')
        else:
    ...
    def __getitem__(self, index):
        ...
        return image, target, index, img_id, caption_label, caption_mask, dv
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids, caption_labels, caption_masks, dv = zip(*data)
    ...
    return images, targets, lengths, ids, img_ids, caption_labels_, caption_masks_, dv

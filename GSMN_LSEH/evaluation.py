...
def main():
    ...
    evalrank("./runs/runX/checkpoint/model_best.pth.tar", data_path="./data/coco", split="test", fold5=False) #model_best.pth.tar model_fliker_2.pth.tar
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, bboxes, depends, lengths, ids, img_ids, lsa) in enumerate(data_loader):
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if data_path is not None:
        ...
        opt.vocab_path = '/home/lunet/coyg4/data/vocab/'
    ...
    if not fold5:
        ...
        sims = shard_xattn(model, img_embs, cap_embs, bbox,
                           depends, cap_lens, opt, shard_size=50)
        end = time.time()
        ...
        for i in range(5):
            ...
            sims = shard_xattn(model, img_embs_shard, cap_embs_shard,
                               bbox_shard, depends_shard, cap_lens_shard, opt, shard_size=50)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def shard_xattn(model, images, captions, bbox, depends, caplens, opt, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                im = Variable(torch.from_numpy(images[im_start:im_end])).cuda().float()
            with torch.no_grad():
                s = Variable(torch.from_numpy(captions[cap_start:cap_end])).cuda().float()

            l = caplens[cap_start:cap_end]
            with torch.no_grad():
                bbx = Variable(torch.from_numpy(bbox[im_start:im_end])).cuda().float()

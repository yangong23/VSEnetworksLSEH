...
def main():
    ...
    lr_schedules = [opt.lr_update, opt.lr_update2, ]
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        if opt.precomp_enc_type == 'basic':
            images, img_lengths, captions, lengths, ids, img_ids, lsa = train_data
            model.train_emb(images, captions, lengths, ids, img_ids, lsa, image_lengths=img_lengths)
        else:
            images, captions, lengths, ids, img_ids, lsa = train_data
            if epoch == opt.embedding_warmup_epochs:
                warmup_alpha = float(i) / num_loader_iter
                model.train_emb(images, captions, lengths, ids, img_ids, lsa, warmup_alpha=warmup_alpha)
            else:
...
def validate(opt, val_loader, model):
    ...
    img_embs = np.array([img_embs[i] for i in range(0, len(img_embs), 1)])
    ...
    currscore1 = r1 + r5 + r10
    currscore2 = r1i + r5i + r10i
    ...
    tb_logger.log_value('Sum of Recall', currscore, step=model.Eiters)
    tb_logger.log_value('Sum of Recall (i2t)', currscore1, step=model.Eiters)
    tb_logger.log_value('Sum of Recall (t2i)', currscore2, step=model.Eiters)

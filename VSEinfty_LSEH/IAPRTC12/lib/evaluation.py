...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            images, image_lengths, captions, lengths, ids, img_ids, lsa = data_i
        else:
            images, captions, lengths, ids, img_ids, lsa = data_i
        ...
        model.forward_loss(img_emb, cap_emb, ids, img_ids, lsa)
